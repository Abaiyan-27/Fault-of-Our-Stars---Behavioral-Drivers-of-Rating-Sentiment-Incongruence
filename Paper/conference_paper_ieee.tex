...
The cleaning process covered missing value removal, duplicate checking, and feature
creation. Date fields were used to extract the year of travel and the number of days
between the visit and when the review was posted. Location text was processed using
fixed rules to identify the province and district. The reviewer's country was
identified from their profile location using a matching dictionary.
...
Travel\_Date        & Date of the visit                                         & Categorical \\
Published\_Date     & Date the review was posted                                & Categorical \\
Rating              & Star rating given (1--5)                                  & Categorical \\
Helpful\_Votes      & Number of users who marked the review as helpful          & Numerical   \\
Title               & Short heading of the review                               & Categorical \\
...
Fine-tuned Cardiff RoBERTa  & 0.733 & 0.677 & 0.758 & Lower stability \\
Fine-tuned RoBERTa-base     & 0.777 & 0.698 & 0.780 & Slightly lower balance \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Once sentiment labels were generated, the review title and text columns were removed.
Three new variables were then built directly from the data without any further
modelling: (i)~\textbf{Incongruent} (yes/no): flags reviews where the star rating
class and predicted sentiment do not match; (ii)~\textbf{Pattern} (six types):
records the direction of the mismatch, for example a positive review with a low
rating; and (iii)~\textbf{Reviewer\_Tier} (four levels): groups reviewers by how
many reviews they have posted — Novice (0--5), Casual (6--20), Active (21--100), and
Expert (101+). This grouping reflects the wide spread in contribution counts
(mean~$= 191.6$, median~$= 54$). Review length and posting delay were transformed
using $\log(1{+}x)$ to correct for skewed distributions.
...
Before modelling, several features were removed: location name and city (too many
unique values), user country and locale (incomplete data), date fields already
covered by derived time variables, and title length (too closely linked to review
length). Travel year was modelled as both a straight line and a curve to check
whether trends changed over time in a non-straight pattern.

Chi-square tests measured how strongly categorical variables relate to incongruence,
with Cram\'{e}r's V reporting effect size. The Mann--Whitney U test was used for
continuous variables. All p-values were corrected for multiple testing using the
Benjamini--Hochberg method (threshold: $q < 0.05$). The 24 predictors were
coded as dummy variables (reference groups: National Parks, Southern Province,
Novice reviewers). A variance inflation check confirmed no problematic overlap
between predictors (max VIF~$= 3.663$). AUC-ROC was the main measure of model
performance as it handles unequal class sizes well.
...
\midrule
1A — Logistic Reg.       & Linear baseline performance     & AUC-ROC       & Class-balanced; 5-fold cross-validation \\
1B — Logit (statsmodels) & Identifies independent drivers  & 95\% CI       & Adjusted odds ratios; controls for confounding \\
2 — Random Forest        & Captures complex relationships  & AUC-ROC       & Detects interactions; supports logit findings \\
SHAP                     & Explains model predictions      & Mean $|\phi|$ & Shows how each feature influences the outcome \\
\bottomrule
\end{tabular}%
}
\end{table}
