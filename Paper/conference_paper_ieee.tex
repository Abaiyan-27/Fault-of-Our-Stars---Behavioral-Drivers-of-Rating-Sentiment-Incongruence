...
If someone is planning to go on a tour to a place, one of the main sources that they look first are reviews by the previous travellers. Usually almost all tourism platforms present both star ratings and the written text sentiments for others to get an understanding about the destinations. In the scope of sentiment analysis, these star ratings are taken as the reference values/labels, which provide an easy mapping to sentiment polarity [1], [2]. But, this assumption hasn’t really been tested much even though there are instances where the star ratings and the text sentiment does not match at all, even introducing noise to the downstream models. The rapid growth of user-generated content on sites like TripAdvisor has greatly increased the amount of review data available [9]. However, the reliability of numerical ratings with respect to the text sentiment have not been analyzed at an equivalent pace.

These divergences in the ratings and texts are actually not errors, but it reflects on how a consistent reviewers’ behaviour is driven by their expectations, experience, and the context. For example, one visitor might write "genuinely fascinating" about a museum but still gives three stars because he expected more, while on the other hand, another might rate a temple five stars out of cultural reverence and might write "it was okay, quite crowded". So, this means the numbers and the words are telling two different stories. In this study, we define this as “sentiment-rating incongruence” and will treat it as a structured behavioral pattern rather than just a problem of how responses get labeled. Many large scale review datasets from previous dataset analysis have proved that a considerable amount of reviews display some form of inconsistency between the sentiment polarity and the given star rating score [5], [6], demonstrating the experimental instability of the assumption of rating-text alignment. Deep learning methods, particularly transformer-based models, have exhibited greater potential to identify these tiny distinctions when compared to traditional lexicon-based approaches [10], [11].

Despite its importance, incongruence remains insufficiently studied. Previous research predominantly uses ratings as proxy labels, concentrating mainly on hotels and restaurants in closely studied areas [2], [12]. Furthermore, topic modelling and aspect-based sentiment analysis have improved our understanding of the relationship between specific service aspects and overall ratings [13], [14]. However, the fundamental issue of systematic incongruence across review types and venue categories has not been properly investigated. Research conducted in Sri Lanka identifies incompatibility between ratings and review texts in hotel reviews [3], [4]. Additionally, broader studies highlight contextual variation and systematic inconsistency [5]–[7].
...
The foundational survey by Pang and Lee [1] has established the sentiment analysis as a traditional area, with the assumption that star ratings actually reflect on the sentiment which was written in text. Even with the recognized imperfections in this assumption, that convention still remains in the tourism research area as a practical labelling strategy and as a reference benchmarking level. Alaei et al. [2] have observed that the star ratings are widely considered as weak labels without a clear validation, with their research being heavily concentrated on hotels and restaurants data. This imbalance is further confirmed by Ameur et al. [12], where they highlighted the limited venue diversity and geographic coverage in existing studies, especially for emerging tourist destinations.

Methodological advances have significantly increased the capabilities of sentiment analysis. Wen et al. [10] demonstrated the effectiveness of the transformer-based models like Bidirectional Encoder Representations from Transformer (BERT) and Enhanced Representation through Knowledge Integration (ERNIE), while Puh and Bagic Babac [11] has shown that predicting both sentiment and the ratings has given more deeper understanding. A multilingual approach along with an aspect based approach will make interpretation much easier [13], [14]. Moreover, some recent zeroshot approaches help to analyze uncommon data without providing any additional information [15].

In the context of Sri Lanka, Abeysinghe and Bandara [3], [4] documented rating-text incompatibility in hotel reviews, but they rely on lexicon-based methods and treat inconsistency primarily as a labeling issue. On the other hand, this study extends the scope to tourism attraction reviews, while making use of transformer-based models, and formulates incongruence as a behavioural aspect requiring context rather than correcting them, and with that, shifting the analytical perspective.

Studies relating to the relationship between ratings and the text are inconclusive. According to Bigne et al. [5], there is a general alignment with some variations based on context, but in contrast, George and Ramos [9] have discovered that ratings tend to be greater than sentiment in most destination related reviews. Kwon et al.[6] further shows that the inconsistency varies with the context and impacts the helpfulness of the reviews.

Reviewer characteristics also play a critical role. Chua and Banerjee [7] show that reviewer expertise affects the relationship between ratings and textual content, whereas the previous work connects sentiment polarity and review depth to perceived usefulness [7], [12], suggesting that ratings and text capture different aspects of the user experience.

In general, however, the problem of sentiment-rating inconsistency is incompletely understood, especially in emerging tourist destinations. While recent advancements in transformer-based methods facilitate large-scale, precise analysis [10], [11], [13] and minimize labeling needs [15], to the best of our knowledge, no existing research attempts to investigate the nature of directionally inconsistent patterns and their fundamental factors using a multi-venue, longitudinal dataset. So, this study tries to address this gap using a large-scale Sri Lankan tourism review dataset spanning diverse attraction types and extended time frame [8], providing new experimental results from an underrepresented context.
