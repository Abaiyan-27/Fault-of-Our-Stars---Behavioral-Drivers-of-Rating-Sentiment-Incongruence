...
This study follows a four-phase process to examine how star ratings and review text
<<<<<<< HEAD
differ in sentiment. The phases are: (1) data screening and preprocessing,
(2) choosing a sentiment model, (3) building analysis variables, and
(4) running statistical and predictive tests. Table~\ref{tab:methodology_overview}
=======
differ in sentiment. Table~\ref{tab:methodology_overview}
>>>>>>> d7b4759 (Revise abstract and methodology sections for clarity; enhance description of sentiment-rating incongruence and analysis process)
gives a summary of the full process.
...
\midrule
<<<<<<< HEAD
1 & Data Screening      & Rule-based normalization; temporal and geographic feature extraction & Clean 16,156-review analytical dataset \\
2 & Model Comparison    & Two pretrained and two fine-tuned RoBERTa variants benchmarked on 1,000 labelled reviews & Sentiment label for each review \\
3 & Variable Building   & Rating grouping, mismatch typology, reviewer-tier and log transforms & Incongruence target and predictors \\
4 & Analysis            & Bivariate tests, logistic models, Random Forest, SHAP & Drivers, effect sizes, predictive performance \\
=======
...
This study uses the ``Tourism and Travel Reviews: Sri Lankan Destinations'' dataset
from Mendeley Data~\cite{ref8}, which contains 16,156 reviews collected between
2010 and 2023. Sri Lanka was chosen because its tourism sector covers a wide range
of venue types, making it a good setting for studying how ratings and text can
differ. As a less-studied destination, it also adds new insight to a research area
dominated by data from Western countries.

Preprocessing first screened the raw file for missing values and duplicate records,
then focused on deterministic feature construction. Date fields were parsed to
extract the year of travel and the number of days between the visit and when the
review was posted, with negative delays clipped at zero. Location text was processed
using fixed rules and manual mappings to identify the province and district. The
reviewer's country was inferred from profile location using an offline-first alias
dictionary and local resolution cache, with locale-based fallback when location text
remained unresolved.
\textit{Review\_Length} was calculated from the review text, and \textit{Rating\_Class}
was created by grouping star ratings into Positive (4--5$\star$), Neutral (3$\star$),
and Negative (1--2$\star$), following common practice~\cite{ref1,ref2}.
Table~\ref{tab:dataset_attributes} lists all 14 original features in the dataset.
...
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{L{2.6cm}L{3.8cm}L{1.6cm}}
\toprule
...
Location            & Raw location text (city, district, province)              & Categorical \\
Location\_Type      & Type of attraction (museum, beach, temple, etc.)          & Categorical \\
User\_ID            & Unique ID for each reviewer                               & Categorical \\
User\_Location      & Reviewer's location (free text)                           & Categorical \\
User\_Locale        & Language or locale setting                                & Categorical \\
User\_Contributions & Total number of reviews posted by the reviewer            & Numerical   \\
Travel\_Date        & Date of the visit                                         & Temporal \\
Published\_Date     & Date the review was posted                                & Temporal \\
Rating              & Star rating given (1--5)                                  & Ordinal \\
Helpful\_Votes      & Number of users who marked the review as helpful          & Numerical   \\
Title               & Short heading of the review                               & Text \\
Text                & Full review content                                       & Text \\
\bottomrule
\end{tabular}%
}
\end{table}
...
<<<<<<< HEAD
To assign a sentiment label (Positive, Neutral, or Negative) to each review, four
transformer-based models were compared using a manually labelled subset of 1,000
reviews. This corpus was partitioned into 700 training reviews and a fixed
300-review test set. Two off-the-shelf checkpoints
(\textit{gosorio/robertaSentimentFT\_TripAdvisor} and
\textit{cardiffnlp/twitter-roberta-base-sentiment}) were evaluated directly on the
held-out test set. Two additional models were fine-tuned on the 700-review training
portion, using an internal stratified 80/20 train/validation split, chunked inputs,
class-weighted loss, and early stopping before final evaluation on the same
300-review test set. The review title and body were joined together as input. For
long reviews, overlapping chunks were created and chunk-level probabilities were
averaged to obtain one review-level prediction. Because the three sentiment classes
were imbalanced, accuracy, macro F1, and weighted F1 were reported together, with
particular attention to macro F1.

As shown in Table~\ref{tab:sentiment_performance}, the pretrained
\textit{cardiffnlp/twitter-roberta-base-sentiment} model performed best overall
(Accuracy $= 0.83$, Weighted F1 $= 0.8051$) and was selected to label all 16,156
reviews. Although fine-tuned RoBERTa-base achieved a marginally higher Macro F1
($0.6984$ vs.\ $0.6918$), it had lower accuracy ($0.7767$ vs.\ $0.83$). Given this
small trade-off and the reproducibility advantage of a fixed pretrained checkpoint,
the Cardiff model was selected for full-corpus inference.
=======
To assign sentiment labels to each review, several transformer-based models were
compared using a manually labelled subset of 1,000 reviews. The review title and
text were combined as input, and Macro F1 was used as the main evaluation metric
due to class imbalance. As shown in Table~\ref{tab:sentiment_performance}, the
pretrained \textit{cardiffnlp/twitter-roberta-base-sentiment} model showed the best
overall balance and was selected to label the full dataset. Its use also improves
reproducibility because it does not require task-specific fine-tuning.
>>>>>>> d7b4759 (Revise abstract and methodology sections for clarity; enhance description of sentiment-rating incongruence and analysis process)
...
Fine-tuned RoBERTa-base     & 0.777 & 0.698 & 0.780 & Higher Mac.\ F1, lower Acc. \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Once sentiment labels were generated, the raw title and text fields were not used as
direct predictors in the downstream statistical models. The analysis variables were
then constructed from model output and reviewer metadata. \textbf{Incongruent}
(yes/no) flags reviews where the star-rating class and predicted sentiment do not
match. \textbf{Pattern} (six types) records the direction of the mismatch, for
example a positive review with a low rating. \textbf{Reviewer\_Tier} groups
reviewers by how many reviews they have posted — Novice (0--5), Casual (6--20),
Active (21--100), and Expert (101+). This grouping reflects the wide spread in
contribution counts (mean~$= 191.6$, median~$= 54$). Review length and posting
delay were transformed using $\log(1{+}x)$ to correct for skewed distributions, and
travel year was centred and squared to capture potential nonlinear time effects.
...
Pattern              & 6-category      & Sentiment $\times$ Rating       & Mismatch typology \\
Reviewer\_Tier       & 4-level ordinal & Contributions (grouped)         & Predictor \\
log\_review\_length  & Continuous      & Review length, $\log(1{+}x)$   & Predictor \\
...
Before modelling, high-cardinality identifiers (location name and city), sparse
user-origin variables (user country and locale), raw date fields already represented
by derived temporal variables, and title length were excluded from the modelling
matrix. Travel year was modelled as both a linear and a quadratic term to check
whether temporal trends changed in a nonlinear way.

Bivariate screening used chi-square tests for categorical predictors and the
Mann--Whitney U test for continuous predictors, with Cram\'{e}r's V reported as the
effect-size measure for venue-type comparisons. Only the five bivariate screening
tests were adjusted for multiple comparisons using the Benjamini--Hochberg method
(threshold: $q < 0.05$). The final explanatory design matrix contained 24
dummy-coded predictors, with National Parks, Southern Province, and Novice
reviewers as the reference groups. A variance inflation check confirmed no
problematic overlap between predictors (max VIF~$= 3.663$). For predictive
evaluation, the full dataset was split once into stratified 80\% training and 20\%
test partitions. Model~1A used class-balanced logistic regression with 5-fold
stratified cross-validation on the training set and AUC-ROC on the common held-out
test set. Model~1B fit an unweighted \texttt{statsmodels} logit to estimate adjusted
odds ratios and 95\% confidence intervals. Model~2 used a Random Forest tuned with
\texttt{GridSearchCV} on the training set only and was then evaluated on the same
held-out test set. SHAP values were computed with TreeExplainer on the best Random
Forest to interpret nonlinear effects. AUC-ROC was the main performance measure
because the incongruence outcome was imbalanced.
Table~\ref{tab:model_framework} describes the three models used.

\begin{table}[htbp]
\caption{Predictive Modelling Framework}
...
1B — Logit (statsmodels) & Identifies independent drivers  & 95\% CI       & Unweighted adjusted odds ratios for inference \\
2 — Random Forest        & Captures complex relationships  & AUC-ROC       & GridSearchCV on training set; same held-out test set \\
SHAP                     & Explains model predictions      & Mean $|\phi|$ & TreeExplainer on the best Random Forest \\
\bottomrule
\end{tabular}%
}
\end{table}
