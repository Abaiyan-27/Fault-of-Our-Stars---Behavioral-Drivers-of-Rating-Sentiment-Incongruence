...
DEFAULT_INPUT_ENCODINGS = ("utf-8", "utf-8-sig", "latin1")
VALID_MODEL_NAMES = set(MODEL_SPECS)
...
def read_input_csv(path: str, encoding: Optional[str]) -> pd.DataFrame:
    if encoding:
        return pd.read_csv(path, encoding=encoding, low_memory=False)

    last_error: Optional[UnicodeDecodeError] = None
    for candidate in DEFAULT_INPUT_ENCODINGS:
        try:
            return pd.read_csv(path, encoding=candidate, low_memory=False)
...
def apply_heuristic_sentiment(
    ...
    """Populate sentiment columns without loading transformer models."""

    text_low = combined_text.str.lower()
...
def compute_sentiment_columns(
    ...
    if disable_models:
        ...
        apply_heuristic_sentiment(df, combined, infer_emotion=True)
        return
    ...
    if not active_sentiment_models:
        ...
        apply_heuristic_sentiment(df, combined, infer_emotion=False)
        return
...
def run_pipeline(
    ...
    df = read_input_csv(input_csv, encoding=input_encoding)
    ...
    for col in REQUIRED_FRAMEWORK_COLUMNS:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--input-encoding",
        default=None,
        help="Optional CSV encoding. Defaults to utf-8, utf-8-sig, then latin1 fallback.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
